...
export function planWindowsFrom(body: unknown): QuotaWindow[] {
  ...
  // 五小时 / 周窗口：rate 或 reset 任一有效就展示（全 0 = 该套餐没有这种窗口）
...
export default {
  ...
  async query({ id, displayName, key, baseUrl, extras }: AdapterQueryInput): Promise<AccountStatus> {
    // 通道一：钱包余额（API key，标准端点；baseURL 带 /step_plan 子路径不影响，计费端点从 origin 推导）
    ...
    // 通道二：Step Plan 套餐点数（控制台 cookie，可选；失败降级为 note 提示）
    ...
    if (consoleCookie !== undefined && consoleCookie !== '') {
      ...
      if (plan !== undefined) windows.push(...plan.windows)
      else planNote = 'Step Plan 点数查询失败：控制台 cookie 可能已失效，请更新 CONSOLE_COOKIE 凭据（控制台请求的整串 Cookie 头）'
    } else {
      planNote = '查看 Step Plan 套餐点数：添加 CONSOLE_COOKIE 凭据（值为控制台请求的整串 Cookie 头）'
    }
    ...
    return account(id, displayName, windows.length > 0 ? 'quota' : 'balance', {
      ...
      note: [
        balances.length === 0 ? '钱包响应里没有 balance 字段' : undefined,
        planNote,
