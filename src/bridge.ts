...
/** pi-ai 来源偏好：设置页「启用最新版 pi-ai」开关。缺省 'dsh'——只用 DSH 自带那份。 */
export type PiAiPreference = 'dsh' | 'latest'
 ...
 * 每次调用现场 resolve（不缓存模块级常量）：测试注入 DSH_HOME 即时生效。
 */
export function safePiAiDir(): string {
...
export function safeInstalledVersions(): string[] {
  const dir = safePiAiDir()
  try {
    return readdirSync(dir)
      .filter((name) => existsSync(join(dir, name, 'package.json'))
        && existsSync(join(dir, name, 'node_modules')))
      .sort(compareVersions)
  } catch {
    return []
  }
 ...
 */
export function readPiAiPreference(status: AnyRecord): PiAiPreference {
 ...
 */
export function safeCandidates(versions: readonly string[]): PiAiCandidate[] {
  return [...versions]
    .reverse()
    .map((version) => ({ key: `safe-${version}`, version, root: join(safePiAiDir(), version), link: true }))
}
 ...
 */
export function obsoleteSafeVersions(versions: readonly string[], chosen: string): string[] {
...
export function probePiAi(requirements: readonly PiAiRequirement[], root: string, key: string): ProbeResult {
  ...
  try {
    ...
    // 一次性脚手架，成败都用完即拆。removeTree 是目录链感知的：探针里的 pi-ai 链只摘
    // 链本身，候选目录一个字节不动。
    removeTree(dir)
  }
}
 ...
 */
export function piAiCandidates(preference?: PiAiPreference): PiAiCandidate[] {
  const list: PiAiCandidate[] = []
  const pref = preference ?? readPiAiPreference(readStatus())
  if (pref === 'latest') list.push(...safeCandidates(safeInstalledVersions()))
  const versions = installedVersions()
...
${restoreHint(host.integrity.version)}`
    ...
    // 选定即清理（只保留最新一个自有版本）：切换已完成——软链已指向 chosen、模块已加载，
    // 这时清旧版当前进程绝不会踩到（懒加载模块要读也是读 chosen）。chosen 不是安全区档时
    // 一个字节都不动：拨 OFF 后保留已下载文件，再拨 ON 零成本。
    if (chosen.key.startsWith('safe-')) {
      for (const stale of obsoleteSafeVersions(safeInstalledVersions(), chosen.version)) {
        removeLinkOrDir(join(safePiAiDir(), stale))
      }
    }
    writeStatus({
