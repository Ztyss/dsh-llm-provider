#!/usr/bin/env bash
# 主线写锁（source 用，不直接执行）：任何会修改 main 分支历史或主线工作区内容的操作
# 都必须先 acquire_main_lock，持锁期间不允许别的进程写 main。
#
# 锁是 <git-common-dir>/main-write.lock 目录：mkdir 原子获取，同一时刻只有一个进程能拿到。
# 拿不到说明已有别的进程在写主线，调用方直接退出（不排队等锁——串行合入的约定是
# 「等上一个完全结束后再跑下一个」，报错退出比静默等待更容易对齐节奏）。
#
# 用法（在脚本里）：
...
#
# 适用/不适用：写 main 分支历史、重建主线产物的操作用锁；只读 main、或只写
# worktree 自己的分支/tag（dev-start / dev-finish）不需要锁。
set -euo pipefail

# 锁目录位置；可用环境变量 MAIN_LOCK_DIR 覆盖（测试用），默认 <git-common-dir>。
_main_lock_dir() {
...
release_main_lock() {
  ...
  # 不 unset LOCK_PATH：路径值保留无害，调用方可继续用它检查持有状态；
