...
- **模型选择器按官方蓝本重写完成**（真机验证）：
  ...
  - 选模型**即关闭菜单**（官方行为，对着 ModelSelect.tsx 核对：`settleSelection` 成功就
    `close(true)`，点当前模型同样关闭）；想接着调档位就重新打开菜单。刷新页面从会话投影
  ...
  - 取数与写回跟官方 ui-model-selection 同一口径（对着 reference/dsh-src 逐条核对）：当前选择取
    `会话投影 next ?? 目录 default`；默认档只认目录声明的 `defaultEffort`，没有就显示「Default」
    （官方的 `effort.providerDefault` 文案，zh/en 字典里都是这个字面值），
    不拿档位表首档顶替；切模型只提交 provider/model，档位由宿主 `resolveCallConfig` 决定并回写投影；
  - 增强项：provider chips 带最小余量指示点（悬停看 5h/7d 明细）、模型行带能力徽章
    （视觉/推理）和上下文标注；**触发器里供应商段后面也带同一个余量指示**（点 + 最紧窗口
    百分比，没有窗口就钱包余额；点按 10%/30% 分红黄绿，悬停看窗口明细与重置倒计时）——
    进页面就拉一次额度、之后每分钟补一次，不用等菜单打开；
  - 触发器放不下时的让位顺序（官方 ModelSelect 那条「档位先缩没」的思路，按我们多出的两段
    扩展）：档位（用户显式选的）和余量一律不收缩，**provider 收缩因子最大、最先让位**，模型名
    最后才截。再不够就按 composer 行宽降级（`@container`，容器就是 InputBar 的 `.row`，跟官方
    PermissionSelect 的 460px 断点同一个）：行 ≤760px 把 provider 段连分隔符一起隐藏，
    行 ≤620px 余量只留彩点，只剩「模型名 · 档位」。宽度上限用行相对的 `min(560px,60cqw)`
    ——官方那个 `min(360px,45cqw)` 是按「模型名 + 档位」两段内容算的，我们四段在宽窗口下就
