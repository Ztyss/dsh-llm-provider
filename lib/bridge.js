 ...
 *
 * 边界：本模块只写插件自己的 vendor/ 目录。vendor/pi-ai/ 空的时候会兜底用 dsh 全局
 * 装的那份 pi-ai（只读——那是别的程序的文件，一个字节都不改，目录补丁在这条路上直接
 * 跳过，见 isVendoredRoot）。
 */
...
export function loadBridge() {
  try {
    ...
    // 4. 目录补丁：**只打我们自己 vendor 里那份**。没有 vendored 版本时 target 是
    //    dsh 全局安装的那份 pi-ai（见上面第 2 步的兜底），那是别的程序的文件，插件
    //    不该写——实测写脏过一次全局安装。兜底时跳过补丁：目录旧一点，好过改别人的包。
    const catalogPatches = isVendoredRoot(target) ? applyCatalogPatches(target) : []
