...
function planQuotaViaCurl(cookie) {
	...
	try {
		...
		if (r.status !== 0) return {
			windows: [],
			note: "Step Plan 点数查询失败：curl 未执行成功（status " + String(r.status) + "）"
		};
...
var stepfun_default = {
	...
	async query({ id, displayName, key, baseUrl, extras }) {
		...
		if (isPlanChannel) {
			...
			if (consoleCookie === void 0 || consoleCookie === "") return account(id, displayName, "quota", {
...
//#endregion
export { stepfun_default as default, planFailureNote, planWindowsFrom };
