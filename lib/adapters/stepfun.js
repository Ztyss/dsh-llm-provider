...
var stepfun_default = {
	...
	async query({ id, displayName, key, baseUrl, extras }) {
		...
		if (isPlanChannel) {
			...
			const seedSha = seed !== void 0 ? hashCookieValue(seed) : void 0;
			const stored = readCookieEntry(ref, {
				path: legacySessionFile(),
				read: readLegacySession
			});
			let cookie;
			if (seed !== void 0 && stored?.seedSha !== seedSha) {
				writeCookieValue(ref, seed, seedSha);
				cookie = seed;
			} else cookie = stored?.value ?? seed;
