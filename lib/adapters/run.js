...
function keyFromCredentials(envName) {
	try {
		const value = readFileSync(join(homedir(), ".dsh", ".credentials.yaml"), "utf8").split("\n").find((l) => l.trim().startsWith(`${envName}:`))?.split(":")[1]?.trim();
...
function resolveKey(adapterId, args) {
	...
	for (const envName of KEY_ENV_NAMES[adapterId] ?? []) {
		...
		if (fromFile !== void 0) return {
			key: fromFile,
			source: `~/.dsh/.credentials.yaml:${envName}`
		};
	}
	return {
		key: void 0,
