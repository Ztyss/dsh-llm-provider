...
//#region src/adapters/opencode-go.ts
/**
...
var opencode_go_default = {
	...
	async query({ id, displayName, key }) {
		const { status, body } = await getJson("https://opencode.ai/zen/go/v1/usage", { authorization: `Bearer ${key}` });
		...
		if (usage !== void 0 && usage !== null) {
			...
			for (const [keyName, label] of WINDOW_LABELS) {
				...
				const percent = typeof window["percent"] === "number" ? window["percent"] : void 0;
				if (windowRaw === void 0 || windowRaw === null || percent === void 0) continue;
				windows.push({
					window: label,
