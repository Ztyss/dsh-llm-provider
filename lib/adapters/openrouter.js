...
var openrouter_default = {
	...
	async query({ id, displayName, key }) {
		const { status, body } = await getJson("https://openrouter.ai/api/v1/credits", { authorization: `Bearer ${key}` });
		...
		const balances = [];
		if (remaining !== void 0) balances.push({
			label: "剩余额度",
			value: formatAmount(remaining, "USD")
		});
		if (total !== void 0) balances.push({
			label: "累计充值",
			value: formatAmount(total, "USD")
		});
		if (used !== void 0) balances.push({
			label: "累计使用",
			value: formatAmount(used, "USD")
		});
