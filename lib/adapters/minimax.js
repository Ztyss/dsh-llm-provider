...
import { account, authFailed, clampPercent, describeHttpError, epochMsToIso, fail, getJson, num, originOf } from "./shared.js";
//#region src/adapters/minimax.ts
/**
...
var minimax_default = {
	...
	async query({ id, displayName, key, baseUrl }) {
		...
		const { status, body } = await getJson(`${origin}/v1/api/openplatform/coding_plan/remains`, { authorization: `Bearer ${key}` });
		...
		if (baseRespRaw !== void 0 && baseRespRaw !== null && num(baseResp["status_code"]) !== 0) fail(`MiniMax 业务错误（code ${String(baseResp["status_code"])}）：${String(baseResp["status_msg"] ?? "未知错误")}`);
		const generalRaw = (Array.isArray(record["model_remains"]) ? record["model_remains"] : []).find((item) => asRecord(item)["model_name"] === "general");
		const windows = [];
		if (generalRaw !== void 0) {
			const general = asRecord(generalRaw);
			const fiveLeft = clampPercent(general["current_interval_remaining_percent"]);
			if (fiveLeft !== void 0) windows.push({
				window: "5 小时窗口",
			...
			if (num(general["current_weekly_status"]) === 1) {
				...
				if (weeklyLeft !== void 0) windows.push({
					window: "每周窗口",
