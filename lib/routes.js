...
const KNOWN_LABELS = {
  ...
  'zai-coding-cn': 'GLM Coding（智谱国内）',
  'qwen-token-plan-cn': 'Qwen Token Plan（国内）',
  'moonshotai-cn': 'Moonshot 开放平台',
}

/** 没在表里的就按 id 拼一个（kimi-coding → "Kimi Coding"）。 */
export function labelOf(providerId) {
  if (KNOWN_LABELS[providerId] !== undefined) return KNOWN_LABELS[providerId]
  return providerId
    .split(/[-_]/)
    .filter((part) => part !== '')
...
/** provider id → 官网/控制台链接（Provider 卡片名称下的跳转链接）。取自 CC Switch 预设（剥掉 aff/utm 跟踪参数）；CC Switch 没有的（moonshot/火山/zenmux）用官方控制台地址。 */
...
export const NATIVE_ROUTE_DEFAULTS = {
  'deepseek-official': { apiKeyEnv: 'DEEPSEEK_API_KEY', label: 'DeepSeek 官方' },
}
 ...
 * @returns `Map<providerId, { id, apiKeyEnv, baseURL, label?, source }>`。
 */
