	...
	factory: (require) => {
		...
		*/
		...
		var LOCAL_DICT = {
			zh: {
				...
				"bridge.stateOff": "使用 DSH 自带版本",
				"bridge.stateOffKept": "已下载 {version}（未启用，拨到 ON 即用）",
				"bridge.stateDownloading": "正在检查/下载上游 pi-ai ...",
				"bridge.statePending": "已下载 {version}，重启 dsh 后生效",
				"bridge.stateRejected": "{version} 未过检验，仍用当前版本",
				"bridge.stateOn": "已启用 {version}（替代 DSH 自带版本）",
				"bridge.featureOff": "pi-ai 更新功能已关闭（DSH_PROVIDER_UPDATE=off）",
			...
			en: {
				...
				"bridge.stateOff": "Using the DSH-bundled version",
				"bridge.stateOffKept": "{version} downloaded (not in use; flip ON to use it)",
				"bridge.stateDownloading": "Checking/downloading upstream pi-ai ...",
				"bridge.statePending": "{version} downloaded; takes effect after a dsh restart",
				"bridge.stateRejected": "{version} failed verification; staying on the current version",
				"bridge.stateOn": "Using {version} (replaces the DSH-bundled copy)",
				"bridge.featureOff": "pi-ai update feature is off (DSH_PROVIDER_UPDATE=off)",
		...
		*/
		function piAiBridgeRows(bridge, piAi) {
			...
			if (piAiRecord.needsRestart === true && safeNewest !== void 0) rows.push({
				key: "pending",
				text: tf("bridge.statePending", { version: safeNewest }),
				warn: true
			});
			...
			if (lastCheck !== void 0 && lastCheck !== null && lastCheck.reason !== void 0) {
				var checkRecord = lastCheck;
				rows.push({
					key: "lastCheck",
					text: String(checkRecord.reason),
					title: checkRecord.at === void 0 ? "" : tf("bridge.lastCheckAt", { when: relativeTime(String(checkRecord.at)) })
				});
			}
			return rows;
		}
		...
		*/
		function piAiUpstreamText(piAi, bridge) {
			...
			if (rec.preference === "latest") {
				if (rec.needsRestart === true && safeNewest !== void 0) return tf("bridge.statePending", { version: safeNewest });
				...
				if (bridgeRecord.active === true && typeof bridgeRecord.source === "string" && bridgeRecord.source.startsWith("safe-")) return tf("bridge.stateOn", { version: bridgeRecord.piAiVersion });
				var lastCheck = rec.lastCheck;
				if (lastCheck !== void 0 && lastCheck !== null && lastCheck.reason !== void 0) return String(lastCheck.reason);
				return t("bridge.stateDownloading");
			}
			if (safeNewest !== void 0) return tf("bridge.stateOffKept", { version: safeNewest });
		...
		function payloadFromRows(list, ctx, fail) {
			...
			for (var i = 0; i < list.length; i += 1) {
				...
				if (row.declared !== void 0) {
					...
					if (row.edited === true) {
						...
						if (ctxRaw !== "") {
							...
							if (ctxVal === void 0) return fail("「" + row.id + "」的上下文窗口要填正整数");
							declared.contextWindow = ctxVal;
						}
						var maxRaw = row.maxTokens.trim();
						if (maxRaw !== "") {
							var maxVal = parsePositiveInt(maxRaw);
							if (maxVal === void 0) return fail("「" + row.id + "」的最大输出要填正整数");
						...
						if (row.reasoning !== void 0) {
							...
							if (row.reasoning === true) {
								...
								if (effortDraft !== void 0) {
									...
									if (effortOut.error !== void 0) return fail("「" + row.id + "」" + effortOut.error);
					...
					if (declared.reasoning === true && declared.reasoningEfforts === void 0 && row.inPiAi !== true) {
						...
						if (repairOut.error !== void 0) return fail("「" + row.id + "」" + repairOut.error);
				...
				if (row.reasoning === true) {
					...
					if (customEffort.error !== void 0) return fail("「" + row.id + "」" + customEffort.error);
		...
		function ProviderSettingsSection() {
			...
			*/
			...
			function togglePiAi(next) {
				...
				postJson("/provider/pi-ai", { enabled: next }).then(function(result) {
					if (result !== null && result !== void 0 && result.disabled === true) {
						setNote(String(result.error));
						setPiAiBusy(false);
						return;
					}
					if (next !== true) {
						setNote(result !== null && result !== void 0 && result.needsRestart === true ? "已拨到「DSH 自带」，重启 dsh 后生效（已下载的文件保留）" : "已在使用 DSH 自带的 pi-ai");
						setPiAiBusy(false);
						refresh(false);
						return;
					}
			...
			*/
			...
			if (status !== null) {
				...
				var badTone = featureDisabled === true || piAiRecord.latestRejected !== void 0 && piAiRecord.latestRejected !== null;
				...
				}), react.default.createElement("span", { key: "text" }, t("bridge.toggle"))), react.default.createElement("span", {
