window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function AddProviderPanel(props) {
			...
			*/
			...
			}, t("prov.cancel"))), test.message === "" ? null : react.default.createElement("div", { className: "plan_note" + (test.phase === "fail" ? " plan_badText" : "") }, test.message), test.phase === "ok" && test.models.length > 0 ? (function() {
		...
		*/
		function prefillEffortsOf(entry, details, providerId) {
		...
		*/
		function ModelListEditor(props) {
			...
			function addFromForm() {
				...
				setRows(function(prev) {
					return prev.concat([{
						...
						effortsDraft: form.effortsDraft !== void 0 ? form.effortsDraft : void 0
					}]);
				});
				setForm(emptyForm);
				setError(null);
			...
			function payloadFrom(list) {
				...
				for (var i = 0; i < list.length; i += 1) {
							...
							if (row.reasoning !== void 0) {
								...
								if (row.reasoning === true) {
									...
									if (effortDraft !== void 0) {
										...
										if (effortOut.error !== void 0) {
											setError("「" + row.id + "」" + effortOut.error);
											return;
										}
					...
					if (row.reasoning === true) {
						...
						if (row.effortsDraft !== void 0) {
							var customEffort = effortsToDeclared(row.effortsDraft, void 0);
							if (customEffort.error !== void 0) {
								setError("「" + row.id + "」" + customEffort.error);
								return;
							}
			...
			*/
			function effortLadderRows(draft, onDraft, keyPrefix) {
				var rows = [];
				for (var i = 0; i < EFFORT_LEVELS.length; i += 1) {
					var level = EFFORT_LEVELS[i];
					var selected = draft[level] !== void 0;
					var wire = draft[level] !== void 0 ? draft[level] : level === "off" ? "" : level;
					rows.push(react.default.createElement("div", {
						className: "pv_meEffLevel",
						key: keyPrefix + "-" + level
					}, react.default.createElement("label", { className: "pv_meEffChip" + (selected ? " pv_meEffOn" : "") }, react.default.createElement("input", {
						type: "checkbox",
						checked: selected,
						onChange: (function(level) {
							return function(event) {
								var next = { ...draft };
								if (event.target.checked === true) next[level] = level === "off" ? "" : draft[level] !== void 0 ? draft[level] : level;
								else delete next[level];
								onDraft(next);
							};
						})(level)
					}), level), selected ? react.default.createElement("input", {
			...
			function effortLadderHint() {
				return react.default.createElement("div", { className: "pv_meEffHint" }, "勾选要提供的档位；线值是实际发给网关的参数（一般与档位同名，可按网关词汇改名）。off = 关闭思考，线值留空表示不发送参数；至少提供一个非 off 档位。");
			}
			function rowEditPanel(row) {
				...
				return react.default.createElement("div", {
					...
					onChange: function(event) {
						...
						if (checked === true && row.effortsDraft === void 0) nextPatch.effortsDraft = prefillEffortsOf(row.declared, props.details, account.id);
						patch(row.id, nextPatch);
					}
				}), "推理"))), row.reasoning === true || row.knownReasoning === true ? react.default.createElement("div", {
					className: "pv_line pv_row",
					key: "p-eff"
				}, react.default.createElement("div", { className: "pv_meEffTitle" }, "思考档位（reasoningEfforts）"), effortLadderHint(), react.default.createElement("div", { className: "pv_meEff" }, effortLadderRows(row.effortsDraft ?? prefillEffortsOf(row.declared, props.details, account.id), function(next) {
					patch(row.id, {
						effortsDraft: next,
						edited: true
					});
				}, "p"))) : null, react.default.createElement("div", {
					className: "pv_meActs",
					key: "p-acts"
				}, react.default.createElement("button", {
					type: "button",
			...
			var formEl = form.open !== true ? null : react.default.createElement("div", { className: "pv_meForm" }, react.default.createElement("div", { className: "pv_meFormTitle" }, "新增自定义模型（保存后才生效）"), formRow("模型 ID", formField("目录里没有的自定义 ID", form.id, "id"), "f-id"), formRow("显示名", formField("留空则同模型 ID", form.name, "name"), "f-name"), formRow("上下文窗口", formField("留空自动按已知模型填", form.ctx, "ctx", true), "f-ctx"), formRow("最大输出", formField("留空自动按已知模型填", form.max, "max", true), "f-max"), react.default.createElement("div", {
				...
				onChange: function(event) {
					var next = event.target.checked === true;
					var patch = { reasoning: next };
					if (next === true && form.effortsDraft === void 0) patch.effortsDraft = prefillEffortsOf(void 0, props.details, account.id);
					setForm(function(prev) {
						return withKeys(prev, patch);
					});
				}
			}), "推理"))), form.reasoning === true ? react.default.createElement("div", {
				className: "pv_line pv_row",
				key: "f-eff"
			}, react.default.createElement("div", { className: "pv_meEffTitle" }, "思考档位（reasoningEfforts）"), effortLadderHint(), react.default.createElement("div", { className: "pv_meEff" }, effortLadderRows(form.effortsDraft ?? prefillEffortsOf(void 0, props.details, account.id), function(next) {
				setForm(function(prev) {
					return withKeys(prev, { effortsDraft: next });
				});
			}, "f"))) : null, react.default.createElement("div", {
				className: "pv_actRow",
				key: "f-acts"
			}, react.default.createElement("button", {
				type: "button",
