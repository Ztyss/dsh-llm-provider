...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css =
      ...
      'background:var(--dsw-specific-menu,var(--dsw-alias-bg-layer-1,#fff));border:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.1));' +
      ...
      '.plan_row:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover-solid,rgba(0,0,0,.04))}' +
      '.plan_row:disabled{cursor:default;opacity:.75}' +
      '.plan_rowCurrent{background:var(--dsw-alias-interactive-bg-hover-solid,rgba(0,0,0,.04))}' +
      ...
      '.plan_bar{margin-top:4px;height:4px;border-radius:2px;background:var(--dsw-alias-bg-layer-2,rgba(0,0,0,.08));overflow:hidden}' +
      ...
      // 微过渡（官方 .16s 节奏）+ 键盘焦点环
      '.plan_row,.pv_pickItem,.pv_iconBtn,.pv_action,.pv_tab,.pv_addBtn,.pv_fclear,.pv_delYes,.pv_delNo,' +
      '.plan_trigger,.mp_chip,.pv_pcLink{transition:background-color .16s ease,color .16s ease}' +
      '.plan_row:focus-visible,.pv_iconBtn:focus-visible,.pv_action:focus-visible,.pv_tab:focus-visible,' +
        ...
        for (var j = 0; j < raw.length; j += 1) {
          ...
          if (typeof model === 'string') models.push({ id: model, name: model })
            ...
            if (model.reasoning !== null && typeof model.reasoning === 'object') {
              var efforts = Array.isArray(model.reasoning.efforts) ? model.reasoning.efforts : []
              var effortIds = []
              for (var r = 0; r < efforts.length; r += 1) {
                var effortId = typeof efforts[r] === 'string' ? efforts[r] : (efforts[r] && efforts[r].id)
                if (typeof effortId === 'string' && effortId !== '') effortIds.push(effortId)
              }
              if (effortIds.length > 0) {
                entry.reasoning = {
                  efforts: effortIds,
                  default: typeof model.reasoning.defaultEffort === 'string' ? model.reasoning.defaultEffort : undefined,
                }
              }
            }
            models.push(entry)
    ...
    /** 模型没显式选强度时的落点：目录默认值，没有则首档。 */
    function defaultEffortOf(model) {
      if (model === null || typeof model !== 'object' || model.reasoning === undefined) return undefined
      if (typeof model.reasoning.default === 'string') return model.reasoning.default
      var efforts = model.reasoning.efforts
      return Array.isArray(efforts) && efforts.length > 0 ? efforts[0] : undefined
    }
    ...
    /** 从客户端缓存里剔除一家（删除 provider 后用：不打上游、不让卡片复活）。 */
    function dropPlanAccount(id) {
      if (planCache.value !== null && planCache.value !== undefined && Array.isArray(planCache.value.accounts)) {
        planCache.value = {
          ...planCache.value,
          accounts: planCache.value.accounts.filter(function (account) {
    ...
    function apiCall(method, args) {
      return fetch('/api/' + method, {
        ...
        .then(function (envelope) {
          ...
          throw new Error(result && result.error ? String(result.error.code) + ': ' + String(result.error.message) : '调用失败')
        })
    }
    ...
    function ModelSwitchSeat(props) {
      ...
      var selection = directoryCurrent !== undefined && directoryCurrent !== null
        ? directoryCurrent
        : (lastSel !== null && lastSel !== undefined ? lastSel : projectionSelection)
      ...
      if (selection !== undefined && selection !== null) {
        for (var gi = 0; gi < groups.length; gi += 1) {
          if (groups[gi].id !== selection.provider) continue
          for (var mi = 0; mi < groups[gi].models.length; mi += 1) {
            if (groups[gi].models[mi].id === selection.model) currentModel = groups[gi].models[mi]
          }
        }
      }
      var reasoning = currentModel !== undefined ? currentModel.reasoning : undefined
      var effectiveEffort = selection !== undefined && selection !== null && typeof selection.reasoningEffort === 'string'
        ? selection.reasoningEffort
        : (reasoning !== undefined ? defaultEffortOf(currentModel) : undefined)
      var effortText = reasoning === undefined
        ? undefined
        : (effectiveEffort === undefined
            ? '服务商默认'
            : (effortLabel(effectiveEffort) ?? effectiveEffort))
      ...
      function chooseModel(groupId, modelId) {
        ...
        for (var i = 0; i < groups.length; i += 1) {
          ...
          for (var j = 0; j < groups[i].models.length; j += 1) {
            if (groups[i].models[j].id === modelId) model = groups[i].models[j]
          }
        }
        var effort = defaultEffortOf(model)
        var req = { provider: groupId, model: modelId }
        if (effort !== undefined) req.reasoningEffort = effort
        // 无推理档位的模型：选完即关；有的：提交后退回根面板，让用户接着调推理等级（官方交互）
        if (effort === undefined) {
          void submit(req)
          return
        }
      ...
      function chooseEffort(effort) {
        ...
        var req = { provider: selection.provider, model: selection.model }
        if (effort !== undefined) req.reasoningEffort = effort
        void submit(req)
      }
      ...
      var effortCell = effortText === undefined
        ? react.createElement(
            'button',
            { type: 'button', className: 'ms_cell', disabled: true, style: { cursor: 'default', opacity: 0.55 } },
            react.createElement('span', { className: 'ms_cellLabel' }, '推理等级'),
            react.createElement('span', { className: 'ms_cellValue' }, '选择模型后可用'),
            react.createElement('span', { className: 'ms_cellChev' }, chevronRightSvg()),
          )
        : react.createElement(
            'button',
            { type: 'button', className: 'ms_cell', onClick: function () { setPane('effort') } },
            react.createElement('span', { className: 'ms_cellLabel' }, '推理等级'),
            react.createElement('span', { className: 'ms_cellValue' }, effortText),
            react.createElement('span', { className: 'ms_cellChev' }, chevronRightSvg()),
          )
      ...
      var visible = []
      for (var vi = 0; vi < groups.length; vi += 1) {
        var group = groups[vi]
        if (providerFilter !== null && group.id !== providerFilter) continue
        for (var vj = 0; vj < group.models.length; vj += 1) {
          var m = group.models[vj]
          if (needle !== '' && fuzzyMatch(query, m.id + ' ' + m.name + ' ' + group.name + ' ' + group.id) !== true) continue
          visible.push({ group: group, model: m })
        }
      }
      var modelPane = null
      if (pane === 'model') {
    ...
    function piAiBridgeRows(bridge, update) {
      ...
      if (bridge.active !== true) {
        rows.push({ key: 'err', text: String(bridge.error), bad: true })
        return rows
      }
      rows.push({
        key: 'pi',
        text: '当前 pi-ai 版本',
        value: String(bridge.piAiVersion) + '（' + piAiSourceLabel(bridge.source) + '）',
        title: piAiSourceHint(bridge.source),
      })
      // 体检没过的候选：为什么没用上更新的那版
      var rejected = Array.isArray(bridge.rejected) ? bridge.rejected : []
      for (var i = 0; i < rejected.length; i += 1) {
        rows.push({
          key: 'skip-' + i,
          text: '跳过 ' + String(rejected[i].version) + '：兼容性检查没通过',
          value: '看原因',
          title: String(rejected[i].error),
          warn: true,
        })
      }
      // 最近一次检查更新的结论
      if (update !== undefined && update !== null) {
        if (update.pending !== undefined) {
          rows.push({ key: 'pending', text: '已下载 ' + String(update.pending) + '，兼容性检查通过，重启 dsh 后生效', warn: true })
        }
        if (update.rejected !== undefined && update.rejected !== null) {
          rows.push({
            key: 'rejected',
            text: String(update.rejected.version) + ' 兼容性检查没通过，已跳过（不会切过去）',
            value: '看原因',
            title: String(update.rejected.error),
            warn: true,
          })
        }
      }
    ...
    function piAiUpstreamText(update) {
      if (update === undefined || update === null || update.latest === undefined) return '上游 未检查'
      var when = update.lastCheck === undefined ? '' : '（检查于 ' + relativeTime(update.lastCheck) + '）'
      return '上游 ' + String(update.latest) + when
    }
    ...
    function AddProviderPanel(props) {
      ...
      function pickPreset(id) {
        ...
        for (var i = 0; i < presets.length; i += 1) {
          if (presets[i].id === id) preset = presets[i]
        }
      ...
      var pickedLabel = form.routeId
      var customPicked = false
      for (var pl = 0; pl < presets.length; pl += 1) {
        if (presets[pl].id === form.routeId) {
          pickedLabel = presets[pl].label
          customPicked = presets[pl].custom === true
        }
      }
      var pickItems = []
        ...
        react.createElement('div', { className: 'pv_pcBody', style: { borderTop: '0', paddingTop: '10px', gap: '6px' } },
              ...
              pickOpen === false
                ...
                : react.createElement(
                    ...
                    react.createElement('input', {
                      ...
                      value: pickFilter,
                      autoFocus: true,
                      onChange: function (event) { setPickFilter(event.target.value) },
                    }),
          ...
          react.createElement(
            ...
            react.createElement('input', {
              ...
              placeholder: 'sk-…',
              value: form.key,
              onChange: function (event) { patchForm({ key: event.target.value }) },
            }),
          ...
          react.createElement(
            ...
            react.createElement('input', {
              ...
              onChange: function (event) { patchForm({ baseURL: event.target.value }) },
            }),
          ),
          react.createElement(
            'div',
            ...
            customPicked
              ? react.createElement(
                  'select',
                  {
                    className: 'pv_field',
                    value: form.api,
                    onChange: function (event) { patchForm({ api: event.target.value }) },
                  },
    ...
    function ProviderSettingsSection() {
      ...
      react.useEffect(
        function () {
          var timer = setInterval(function () {
            setNowTick(function (n) { return n + 1 })
          }, 30000)
          return function () {
            clearInterval(timer)
          }
      ...
      function onProviderAdded() {
        refresh(true)
        setCatTick(function (t) { return t + 1 })
        reloadPresets()
      }
      ...
      function onProviderRemoved(account) {
        ...
        setPlan(function (prev) {
          ...
          return { ...prev, accounts: prev.accounts.filter(function (entry) { return entry.id !== account.id }) }
        })
        setCatTick(function (t) { return t + 1 })
        reloadPresets()
      }
        ...
        fetch('/provider/refresh', {
          ...
          .then(function (res) {
            if (res !== null && res !== undefined && res.account !== undefined) {
              ...
              showToast('✓ ' + shortName(account) + ' 余量已刷新' + refreshSummary(fresh), true)
              return
            }
    ...
    function apply(ctx) {
      ...
      try {
        ...
        recordDiagnostic('locale', String(cause && cause.message ? cause.message : cause))
      }
