window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function indexModelDetails(models) {
			var map = {};
			...
			for (var i = 0; i < models.length; i += 1) {
				...
				map[detailKeyOf(record.provider, record.id)] = record;
			}
			return map;
		}
		...
		function loadModelDetailMap() {
			return getJson("/provider/models").then(function(payload) {
				return indexModelDetails(payload === null || payload === void 0 ? void 0 : payload.models);
			});
		}
		...
		*/
		function piAiBridgeRows(bridge, update) {
			var rows = [];
		...
		/** 上游那一行的文字（右侧按钮由组件补）。 */
		function piAiUpstreamText(update) {
		...
		*/
		...
		function detailOf(detailsById, provider, modelId) {
			if (detailsById === void 0 || detailsById === null) return void 0;
		...
		*/
		function modelEditorRow(row, routeId, editor, update, expanded, toggleExpand) {
			var expandKey = routeId + ":" + row.id;
			var isOpen = expanded[expandKey] === true;
			var children = [react.default.createElement("div", {
				className: "pv_edRow",
				key: "head"
			}, react.default.createElement("input", {
				type: "checkbox",
				...
				onChange: function(ev) {
					var next = ev.target.checked === true;
					update(routeId, {
						...editor,
						rows: patchModelRow(editor.rows, row.id, { served: next })
					});
				}
			}), react.default.createElement("span", {
				className: "pv_edId",
				title: row.id
			}, row.id), row.source === "declared" ? react.default.createElement("span", { className: "pv_edTag" }, "自定义") : null, react.default.createElement("button", {
				type: "button",
			...
			if (isOpen) children.push(react.default.createElement("div", {
				className: "pv_edFields",
				key: "fields"
			}, react.default.createElement("label", { className: "pv_edField" }, "名称", react.default.createElement("input", {
				className: "pv_field",
				value: row.name,
				onChange: function(ev) {
					update(routeId, {
						...editor,
						rows: patchModelRow(editor.rows, row.id, { name: ev.target.value })
					});
				}
			})), react.default.createElement("label", { className: "pv_edField" }, "上下文窗口", react.default.createElement("input", {
				className: "pv_field",
				placeholder: "留空 = 用默认值",
				...
				onChange: function(ev) {
					update(routeId, {
						...editor,
						rows: patchModelRow(editor.rows, row.id, { contextWindow: ev.target.value })
					});
				}
			})), react.default.createElement("label", { className: "pv_edField" }, "最大输出", react.default.createElement("input", {
				className: "pv_field",
				placeholder: "留空 = 用默认值",
				value: row.maxTokens,
				onChange: function(ev) {
					update(routeId, {
						...editor,
						rows: patchModelRow(editor.rows, row.id, { maxTokens: ev.target.value })
					});
				}
			})), react.default.createElement("div", { className: "pv_edField" }, "输入模态", react.default.createElement("span", { className: "pv_hint" }, row.input === void 0 || row.input.length === 0 ? "未知（不写这个字段）" : row.input.join(" + ")))));
			return react.default.createElement("div", {
				className: "pv_edItem" + (row.served ? "" : " pv_edItemOff"),
				key: "ed-" + row.id
			}, children);
		}
		/**
		...
		*/
		function ProviderSettingsSection() {
			...
			var delState = react.default.useState({});
			var delConfirm = delState[0];
			var setDelConfirm = delState[1];
			var backupState = react.default.useState({});
			...
			/** 覆盖某个 provider 的编辑态。 */
			function updateModelEditor(routeId, next) {
				setModelEditors(function(prev) {
					return withKey(prev, routeId, next);
				});
			}
			/** 行级展开/收起（key = routeId:modelId）。 */
			function toggleEditorRow(key) {
				setEditorOpen(function(prev) {
			...
			*/
			function saveModelList(account, rows) {
			...
			*/
			function resetModelList(account) {
				setNote("正在恢复目录默认 …");
				apiCall("settings/mutate", {
					ns: "llm-pi-ai",
					ops: [{
			...
			*/
			function exportRoute(account) {
				...
				if (clipboard === void 0 || typeof clipboard.writeText !== "function") {
					setBackups(function(prev) {
						return withKey(prev, account.id, tf("del.clipboardBlocked", { text }));
					});
					return;
				}
				clipboard.writeText(text).then(function() {
					setBackups(function(prev) {
						return withKey(prev, account.id, "copied");
					});
				}, function(cause) {
					setBackups(function(prev) {
			...
			function toggleDeleteMode(id, on) {
				setDelConfirm(function(prev) {
					return withKey(prev, id, on);
				});
				setBackups(function(prev) {
					return withKey(prev, id, void 0);
				});
			}
			function removeProvider(account) {
				postJson("/provider/remove", { providerId: account.id }).then(function(res) {
					...
					setBackups(function(prev) {
						return withKey(prev, account.id, void 0);
					});
			...
			*/
			...
			var bridgeRows = piAiBridgeRows(status === null || status.bridge === void 0 ? void 0 : status.bridge, status === null || status.update === void 0 ? void 0 : status.update);
			var bridgeLines = [];
			...
			bridgeLines.push(react.default.createElement("div", {
				className: "pv_line",
				key: "action"
			}, t("bridge.hostOnly"), react.default.createElement("button", {
				type: "button",
			...
			for (var i = 0; i < accounts.length; i += 1) (function(account) {
				...
				if (expanded) {
					...
					if (models === void 0) bodyRows.push(react.default.createElement("div", {
					...
					else if (models.length === 0) bodyRows.push(react.default.createElement("div", {
						className: "pv_line",
						key: "m-none"
					}, t("prov.noModels")));
					else {
						...
						if (modelsOpen) {
							...
							var editor = modelEditors[account.id] === void 0 ? buildModelEditor(account.id, models, detailsById, route === void 0 ? void 0 : route.models) : modelEditors[account.id];
							var editorRows = editor.rows;
							var servedCount = 0;
							for (var sr = 0; sr < editorRows.length; sr += 1) if (editorRows[sr].served) servedCount += 1;
							var edRows = [];
							edRows.push(react.default.createElement("div", {
								className: "pv_edHead",
								key: "ed-head"
							}, react.default.createElement("span", null, "模型清单（" + String(servedCount) + "/" + String(editorRows.length) + " 已提供）"), react.default.createElement("span", { className: "pv_edMode" }, editor.mode === "custom" ? "自定义清单" : "目录默认")));
							for (var er = 0; er < editorRows.length; er += 1) edRows.push(modelEditorRow(editorRows[er], account.id, editor, updateModelEditor, editorOpen, toggleEditorRow));
							edRows.push(react.default.createElement("div", {
								className: "pv_edAdd",
								key: "ed-add"
							}, react.default.createElement("input", {
								...
								onClick: function() {
									updateModelEditor(account.id, {
										...editor,
										rows: addModelRow(editorRows, editor.pendingId),
										pendingId: ""
									});
								}
							}, "添加")));
							...
							edRows.push(react.default.createElement("div", {
								...
								type: "button",
								className: "pv_action",
								disabled: editorError !== void 0,
								title: editorError === void 0 ? "" : editorError,
								onClick: function() {
									saveModelList(account, editorRows);
								}
							}, "保存清单"), route !== void 0 && route.models !== void 0 ? react.default.createElement("button", {
								type: "button",
								className: "pv_action",
								onClick: function() {
									resetModelList(account);
								}
							}, "恢复目录默认") : null, editorError === void 0 ? null : react.default.createElement("span", { className: "plan_note plan_badText" }, editorError)));
							mBoxRows.push(react.default.createElement("div", {
								className: "pv_modelEditor",
								key: "ed"
							}, edRows));
						}
						bodyRows.push(react.default.createElement("div", {
					...
					if (account.deletable === true && delConfirm[account.id] === true) {
						...
						var delRows = [react.default.createElement("div", {
							className: "pv_delPanelTitle",
							key: "t"
						}, tf("del.title", { id: account.id })), react.default.createElement("div", {
							className: "pv_delPanelBody",
							key: "b"
						}, typeof account.apiKeyEnv === "string" && account.apiKeyEnv !== "" ? tf("del.bodyWithRef", { ref: account.apiKeyEnv }) : t("del.body"))];
						if (backup === "copied") delRows.push(react.default.createElement("div", {
							className: "plan_note",
							key: "ok"
						}, t("del.copied")));
						else if (typeof backup === "string" && backup !== "") delRows.push(react.default.createElement("div", {
							className: "plan_note plan_badText",
							key: "fail"
						}, backup));
						delRows.push(react.default.createElement("div", {
							className: "pv_delPanelActs",
							key: "a"
						}, react.default.createElement("button", {
							type: "button",
							className: "pv_delYes",
						...
						}, t("del.confirm")), react.default.createElement("button", {
							type: "button",
							className: "pv_delNo",
							onClick: function() {
								exportRoute(account);
							}
						}, t("del.export")), react.default.createElement("button", {
							type: "button",
							className: "pv_delNo",
							onClick: function() {
								toggleDeleteMode(account.id, false);
							}
						}, t("prov.cancel"))));
						bodyRows.push(react.default.createElement("div", {
							className: "pv_delPanel",
							key: "del"
						}, delRows));
					}
				}
				...
				}, "✎"), account.deletable === true ? delConfirm[account.id] === true ? null : react.default.createElement("button", {
					type: "button",
					className: "pv_iconBtn",
					title: t("prov.removeTip"),
					onClick: function() {
						toggleDeleteMode(account.id, true);
					}
			...
			}), cards), toast === null ? null : react.default.createElement("div", { className: "pv_toast " + (toast.ok === true ? "pv_toastOk" : "pv_toastFail") }, toast.text));
		}
		//#endregion
		...
		*/
		...
		exports.detailKeyOf = detailKeyOf;
		exports.dropPlanAccount = dropPlanAccount;
		exports.headlineChips = headlineChips;
		exports.indexModelDetails = indexModelDetails;
