window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		var LOCAL_DICT = {
			...
			en: {
				...
				"quota.accountNotice": "account unavailable",
		...
		*/
		...
		function quotaTipOf(account) {
			...
			for (var i = 0; i < windows.length; i += 1) {
				...
				var text = label === t("win.remain") ? tf("quota.headlineNoLabel", { percent: windows[i].percentLeft }) : tf("quota.headlineRemaining", {
					label,
		...
		*  档位顺序固定，上游返回顺序变化或同一档出现多次时分割线位置不会跳。 */
		...
		function headlineChips(account) {
			...
			for (var ai = 0; ai < alerts.length; ai += 1) {
				...
				if (alertRow.key === "err") alertChips.push({
					text: t("quota.queryFailed"),
					percent: 0,
					tip: alertRow.text
				});
				else if (alertRow.key === "warn") alertChips.push({
					text: t("quota.credentialAlert"),
					percent: 0,
					tip: alertRow.text
				});
				else alertChips.push({
					text: t("quota.accountNotice"),
					percent: 20,
					tip: alertRow.text
				});
			}
		...
		*/
		function providerAlerts(account) {
			if (account === void 0 || account === null) return [];
			var rows = [];
		...
		*/
		function AddProviderPanel(props) {
			...
			*/
			...
			var fieldEditable = pickedPreset !== void 0;
			function roField(text) {
				return react.default.createElement("span", { className: "pv_field pv_ro" }, text);
			}
			var pickItems = [];
			...
			return react.default.createElement("div", { className: "pv_pc" }, react.default.createElement("div", {
			...
			}, react.default.createElement("span", null, pickedLabel === "" ? t("prov.selectPlaceholder") : pickedLabel), react.default.createElement("span", { className: "pv_selChev" }, caretSvg(pickOpen))), pickOpen === false ? null : react.default.createElement("div", { className: "pv_pickMenu" }, react.default.createElement("input", {
			...
			}), react.default.createElement("div", { className: "pv_pickList" }, pickItems)))), react.default.createElement("div", { className: "pv_line pv_row" }, react.default.createElement("span", null, t("prov.routeId")), customPicked !== true ? roField(form.routeId) : react.default.createElement("input", {
			...
			})), react.default.createElement("div", { className: "pv_line pv_row" }, react.default.createElement("span", null, t("prov.apiKey")), fieldEditable !== true ? roField("") : react.default.createElement("input", {
				className: "pv_field pv_key",
				type: "password",
				value: form.key,
				onChange: function(event) {
				...
				onClick: runTest
			}, test.phase === "run" ? t("prov.discovering") : t("prov.discover")), form.websiteUrl === void 0 ? null : react.default.createElement("a", {
				className: "pv_pcLink",
				href: form.websiteUrl,
				target: "_blank",
				rel: "noreferrer",
				style: { marginLeft: "8px" }
			}, t("prov.keyLink"))), react.default.createElement("div", { className: "pv_line pv_row" }, react.default.createElement("span", null, t("prov.apiBase")), fieldEditable === true && pickedPreset !== void 0 && String(pickedPreset.baseURL ?? "") === "" ? react.default.createElement("input", {
				className: "pv_field pv_key",
				value: form.baseURL,
				onChange: function(event) {
					patchForm({ baseURL: event.target.value });
				}
			}) : roField(form.baseURL), react.default.createElement("button", {
				type: "button",
				className: "pv_action",
				style: { marginLeft: "0" },
			...
			}, t("prov.queryConfig"))), react.default.createElement("div", { className: "pv_line pv_row" }, react.default.createElement("span", null, t("prov.protocol")), customPicked ? pvSelectView({
				open: apiOpen,
				value: form.api,
				options: PROVIDER_API_OPTIONS.map(function(option) {
					return {
						value: option,
						label: providerApiLabel(option)
					};
				}),
				onToggle: function() {
					setApiOpen(!apiOpen);
			...
			}) : roField(providerApiLabel(form.api))), react.default.createElement("div", { className: "pv_actRow" }, react.default.createElement("button", {
				type: "button",
				className: "pv_action",
				style: { marginLeft: "0" },
		...
		*/
		...
		exports.providerAlerts = providerAlerts;
