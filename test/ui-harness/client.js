	...
	factory: (require) => {
		...
		*/
		function modelEditRow(row, patch, remove) {
			...
			}), row.enabled === true || known === false ? react.default.createElement("button", {
				type: "button",
				className: "pv_iconBtn",
				title: known ? "从已配置清单里删除这个模型（保存后生效）" : "把这行自定义条目从清单里去掉",
				onClick: function() {
					remove(row.id);
				}
			}, "✕") : react.default.createElement("span", { key: "del-slot" }));
		}
		...
		function buildEditRows(account, catalog, details) {
			...
			function add(id, name, detail, entry) {
				...
				rows.push({
					id,
					name,
					enabled: declared.some(function(item) {
						return item.id === id;
					}),
		...
		function ModelListEditor(props) {
			...
			*/
			function payload() {
				...
				if (out.length === 0) {
					setError("至少留一个模型；要让这家回到「目录全量」请点「跟随目录（还原）」");
					return;
				}
				return out;
			}
			...
			return react.default.createElement("div", { className: "pv_me" }, react.default.createElement("div", { className: "pv_hint" }, declaredCount > 0 ? "当前只服务清单里的 " + String(declaredCount) + " 个模型。" : "当前跟随 pi-ai 目录（" + String(rows.length) + " 个可用）。"), react.default.createElement("div", { className: "pv_meList" }, [colHead].concat(rows_)), react.default.createElement("div", { className: "pv_meActs" }, react.default.createElement("button", {
				type: "button",
				className: "pv_action",
				style: { marginLeft: "0" },
				disabled: busy,
			...
			}, "跟随目录（还原）")), formEl, error === null ? null : react.default.createElement("div", { className: "plan_note plan_badText" }, error));
		}
		/**
		...
		*/
		function ProviderSettingsSection() {
			...
			*/
			...
			}, bridgeLines, !note ? null : react.default.createElement("div", { className: "plan_note" }, note))) : usageWait === true && plan === null ? react.default.createElement("div", { className: "pv_pc" }, react.default.createElement("div", { className: "pv_pcBody pv_usageLoading" }, react.default.createElement("span", { className: "pv_spin" }, "↻"), react.default.createElement("span", null, t("prov.usageLoading")))) : react.default.createElement("div", { style: {
				display: "flex",
				flexDirection: "column",
				gap: "10px"
